...
export const RecipeDisplay = ({ recipe, isLoading, onRegenerate }: RecipeDisplayProps) => {
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            ...
            <div className="text-center space-y-2">
              <p className="text-lg font-semibold">Creating your recipe...</p>
  ...
  return (
    <Card className="w-full overflow-hidden animate-fade-in bg-card/80 backdrop-blur-sm">
      {/* Recipe Image */}
      {recipe.imageUrl && (
        <div className="relative h-64 md:h-80 overflow-hidden rounded-t-2xl">
          ...
          <div className="absolute bottom-4 left-4 right-4">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-white drop-shadow-lg">
              {recipe.name}
            </h2>
          </div>
        </div>
        ...
        <div>
          ...
          <ol className="space-y-4">
            {recipe.instructions.map((instruction, index) => (
              ...
              >
                <span className="shrink-0 w-8 h-8 rounded-full bg-muted text-foreground font-semibold text-sm flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                  {index + 1}
                </span>
